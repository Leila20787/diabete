...
#'function returns a dataframe contains the number of women who have diabetes with normal insulin levels and the number
#' women who have diabetes with abnormal insulin levels
#' @param data
#' @export
Diab_IS=function(data){
  cpt<-0
  cpt1<-0
  for (i in 1:nrow(data)){

    if (data$Insulin[i] < 120 & data$Insulin[i]>0 & data$Outcome[i] == 1 ) {cpt=cpt+1}

    if( data$Insulin[i] > 120 & data$Outcome[i] == 1 ){cpt1=cpt1+1}

  };
  liste2 <- list(cpt,cpt1)
  data <- data.frame(liste2)
  rownames(data) <- "nb"
  colnames(data) <-c("IS_N","IS_A")
  print(data) # retourne un data.frame
